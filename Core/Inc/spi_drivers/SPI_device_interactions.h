...
/**
 * @brief Initialize SEN0306 mmWave radar sensor with DMA.
 * FIXED: Takes UART handle, not state struct.
 */
void sen0306_init(UART_HandleTypeDef *huart);

/**
 * @brief UART Rx Event Callback (DMA Idle/Half/Full).
 * FIXED: Replaces the old byte-by-byte ISR.
 * Call this from HAL_UARTEx_RxEventCallback.
 */
