...
// ---------------------Change-------------------------
#include "FreeRTOS.h"
#include "codec_test_data.pb.h" // CodecTestData struct + CodecTestData_fields (generated by nanopb)
#include "gnss_radio_master.h"
#include "rp/codec.h" // rp_packet_decode(), rp_packet_decode_result_t, RP_CODEC_OK
#include "task.h"
#include <stdint.h>
// ----------------------------------------------------
  ...
  for (;;) {
    ...
    if (flags & GNSS_RADIO_MSG_READY_FLAG) {
      ...
      while (gnss_radio_dequeue(spi_msg)) {
        ...
        // TODO: debug by printing the struct (dec) to UART1
      }
    }
    // ----------------------------------------------------
