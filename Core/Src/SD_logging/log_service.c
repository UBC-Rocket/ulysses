...
void log_service_log_state(const state_t *state, flight_state_t flight_state) {
  ...
  log_record_state_snapshot_t snapshot = {.timestamp_us = state->u_s,
                                          .q_w = state->q_bn.w,
                                          .q_x = state->q_bn.x,
                                          .q_y = state->q_bn.y,
                                          .q_z = state->q_bn.z,
                                          ...
                                          .estop_active =
                                              (uint8_t)(flight_state == E_STOP),
                                          .reserved = 0U};

  log_writer_append_record(LOG_RECORD_TYPE_state_snapshot, &snapshot,
                           sizeof(snapshot));
}
...
void log_service_log_flight_header(uint32_t timestamp_us, uint32_t flight_magic,
  ...
  log_record_flight_header_t header = {.timestamp_us = timestamp_us,
                                       .flight_magic = flight_magic,
                                       .flight_counter = flight_counter};

  log_writer_append_record(LOG_RECORD_TYPE_flight_header, &header,
                           sizeof(header));
}
...
void log_service_log_accel_sample(uint32_t timestamp_us, float ax_mps2,
  ...
  log_writer_append_record(LOG_RECORD_TYPE_accel_sample, &record,
                           sizeof(record));
}
...
void log_service_log_gyro_sample(uint32_t timestamp_us, float gx_rad_s,
  ...
  log_writer_append_record(LOG_RECORD_TYPE_gyro_sample, &record,
                           sizeof(record));
}
...
void log_service_log_baro_sample(uint32_t timestamp_us, int32_t temp_centi,
  ...
  log_record_baro_sample_t record = {.timestamp_us = timestamp_us,
                                     .temp_centi = temp_centi,
                                     .pressure_centi = pressure_centi,
                                     .seq = seq};

  log_writer_append_record(LOG_RECORD_TYPE_baro_sample, &record,
                           sizeof(record));
}
...
void log_service_log_baro2_sample(uint32_t timestamp_us, int32_t temp_centi,
  ...
  log_record_baro2_sample_t record = {.timestamp_us = timestamp_us,
                                      .temp_centi = temp_centi,
                                      .pressure_centi = pressure_centi,
                                      .seq = seq};

  log_writer_append_record(LOG_RECORD_TYPE_baro2_sample, &record,
                           sizeof(record));
}
...
void log_service_log_event(uint16_t event_code, uint16_t data,
  ...
  log_record_event_t event = {
      .timestamp_us = timestamp_us, .event_code = event_code, .data_u16 = data};

  log_writer_append_record(LOG_RECORD_TYPE_event, &event, sizeof(event));
}
...
void log_service_log_radio_telemetry(uint32_t timestamp_ms,
  ...
  log_record_radio_telemetry_t record = {.timestamp_ms = timestamp_ms,
                                         .ang_rate_x_rps = ang_rate_x_rps,
                                         .ang_rate_y_rps = ang_rate_y_rps,
                                         .ang_rate_z_rps = ang_rate_z_rps,
                                         .thrust_cmd = thrust_cmd,
                                         .gimbal_x_rad = gimbal_x_rad,
                                         .gimbal_y_rad = gimbal_y_rad};

  log_writer_append_record(LOG_RECORD_TYPE_radio_telemetry, &record,
                           sizeof(record));
}
...
void log_service_log_status(uint32_t timestamp_ms, uint32_t uptime_ms,
  ...
  log_record_status_t record = {.timestamp_ms = timestamp_ms,
                                .uptime_ms = uptime_ms,
                                .flight_state = flight_state,
                                .accel_ok = (uint8_t)accel_ok,
                                .gyro_ok = (uint8_t)gyro_ok,
                                .baro1_ok = (uint8_t)baro1_ok,
                                .baro2_ok = (uint8_t)baro2_ok,
                                .gps_connected = (uint8_t)gps_connected,
                                .reserved = 0U,
                                .radio_tx_count = radio_tx_count,
                                .radio_rx_count = radio_rx_count,
                                .cmd_rx_count = cmd_rx_count};

  log_writer_append_record(LOG_RECORD_TYPE_status, &record, sizeof(record));
}
...
void log_service_log_controls_output(uint32_t timestamp_us, float T_cmd,
  ...
  log_record_controls_output_t record = {.timestamp_us = timestamp_us,
                                         .T_cmd = T_cmd,
                                         .theta_x_cmd = theta_x_cmd,
                                         .theta_y_cmd = theta_y_cmd,
                                         .esc_thrust_0 = esc_thrust_0,
                                         .esc_thrust_1 = esc_thrust_1,
                                         .servo_x_deg = servo_x_deg,
                                         .servo_y_deg = servo_y_deg,
                                         .flight_state = flight_state,
                                         .flags = flags,
                                         .reserved = 0U};

  log_writer_append_record(LOG_RECORD_TYPE_controls_output, &record,
                           sizeof(record));
}
